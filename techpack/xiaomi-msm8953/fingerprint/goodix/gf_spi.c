...
static inline int gf_parse_dts(struct gf_dev *gf_dev) {
	...
	if (!IS_ERR(pinctrl)) {
		state = pinctrl_lookup_state(pinctrl, "goodixfp_spi_active");
		if (!IS_ERR(state))
			pinctrl_select_state(pinctrl, state);
		state = pinctrl_lookup_state(pinctrl, "goodixfp_reset_active");
		if (!IS_ERR(state))
			pinctrl_select_state(pinctrl, state);
		state = pinctrl_lookup_state(pinctrl, "goodixfp_irq_active");
		if (!IS_ERR(state))
			pinctrl_select_state(pinctrl, state);
	}

	gf_dev->reset_gpio =
		of_get_named_gpio(dev->of_node, "goodix,gpio_reset", 0);
	if (gpio_is_valid(gf_dev->reset_gpio)) {
		devm_gpio_request(dev, gf_dev->reset_gpio, "goodix,gpio_reset");
		gpio_direction_output(gf_dev->reset_gpio, 1);
	}
	gf_dev->irq_gpio =
		of_get_named_gpio(dev->of_node, "goodix,gpio_irq", 0);
